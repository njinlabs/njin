...
const auth = new Hono<Njin>()
  .post("/", validator("json", createTokenValidation), async (c) => {
    const { email, password } = await c.req.valid("json");

    try {
      const user = await User.findOneByOrFail({ email: email.toLowerCase() });
  ...
  .get("/", authMiddleware("user"), (c) => {
    return c.json(c.var.auth.user?.serialize());
  })
  .delete("/", authMiddleware("user"), async (c) => {
