...
paymentMethod.get(
  ...
  async (c) => {
    ...
    return c.json(response("Payment method result", paymentMethod.serialize()));
  }
);
...
paymentMethod.get(
  ...
  async (c) => {
    ...
    return c.json(response("Payment method result", result.data, result.meta));
  }
);
